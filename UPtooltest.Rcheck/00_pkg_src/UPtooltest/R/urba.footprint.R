...
#'
#' METHOD:
#' Urban footprint (urban.footprint) of the urban area in a specific year or scenario is calculated
#' by adding up the area of each pixel with buildup (r), then divide it by 1e6 to convert the value to square kilometer.
#'
#' @param ras one raster that contains buildup information
#' @return a data frame with information about the urban footprint area in square kilometers
#' @import raster
#' @export
#' @examples
#' library(raster)
...
urban.footprint <- function(ras){
  ...
  x <- lapply(1:nlayers(ras), function(i){
    ...
    t <- data.frame(indicator = "Urban footprint",
                    fclass = names(r),
