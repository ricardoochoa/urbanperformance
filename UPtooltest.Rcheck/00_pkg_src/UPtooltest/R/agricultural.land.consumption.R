#' This function calculates the amount of high-value agricultural land that is estimated
#' to become urban settlements, between the base year and the horizon year.
#'
#' METHOD:
#' Agricultural land consumption (agricultural.consumption) is calculated as the area of the
#' urban footprint (buildup) that was land with high agricultural value in the base year.
#' The first step is to define the polygon or the area that acknowledges high-value agricultural land.
#' The agricultural land lost to urbanization (agri.c) is calculated by adding up the hectares of urban area (buildup)
#' located within the agricultural polygon (agri.b).
#'
#' @param agri  polygon or raster that contains the area of high-value agricultural land
#' @param buildup raster that contains the urban footprint in the base year
#' @return a data frame with information about the losses in agricultural land in square kilometers
#' @import raster
#' @import dplyr
#' @import sf
#' @export
...
agricultural.land.consumption <- function(agri, buildup) {
  ...
  agri.c[agri.c != 2] <- NA
  agri.r <- projectRaster(agri.c, crs = CRS("+init=EPSG:3857"))
