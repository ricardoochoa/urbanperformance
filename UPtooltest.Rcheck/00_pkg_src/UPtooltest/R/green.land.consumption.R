#' This function calculates the amount of valuable natural land that is estimated
...
#' urban footprint (buildup) that was land with valuable green land in the base year.
#' The first step is to define the polygon or the area that acknowledges valueable land.
#' The green land lost to urbanization (greenl.c) is calculated by adding up the hectares of urban area (buildup)
#' located within the green land polygon (greenl.b).
#'
#' @param greenl  polygon or raster that contains the area of valuable green land
#' @param buildup raster that contains the urban footprint in the base year
#' @return a data frame with information about the losses in green land in square kilometers
#' @import raster
#' @import dplyr
#' @export
#' @examples
...
green.land.consumption <- function(greenl, buildup) {
  ...
  greenl.r <- projectRaster(greenl.c, crs = CRS("+init=EPSG:3857"))
