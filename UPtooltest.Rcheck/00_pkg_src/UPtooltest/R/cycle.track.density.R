...
#'
#' METHOD:
#' The cycling infrastructure coverage (cycle.dens) is calculated by adding up the
#' length of each cycle path (c.lengthm) in the urban area and dividing this total (c.length) by
...
#' @return a data frame with information about the total km of cycle tracks per square kilometer
#' @import raster
#' @import dplyr
#' @import sf
#' @export
...
cycle_track_density <- function(cycle, footprint){
  ...
  cycle.dens <- data.frame(indicator = "Cycle density",
