...
#'
#' METHOD:
#' The park and public space area available per person (green.area.p) is estimated
#' by adding up the area with vegetation (green.area), in square meters, in the parks
#' and public open spaces of the urban area, and dividing this result (green.area.t) by the total population (t.pop).
#'
#' @param ga  raster layer or shapefile that contains the polygons for green areas
#' @param pop raster of population distribution or numeric value with information of total population
...
green.area.pcapita <- function(ga, pop) {
  ...
  if (inherits(ga, "RasterLayer")) {
    ...
    green.area.r <- projectRaster(ga, crs = CRS("+init=EPSG:3857"))
